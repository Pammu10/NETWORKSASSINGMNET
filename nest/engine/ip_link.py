...
def add_wg_interface(ns_name, dev_name):
    ...
    exec_subprocess(f"ip netns exec {ns_name} ip link add dev {dev_name} type wireguard")


def remove_wg_interface(ns_name, dev_name):
    ...
    exec_subprocess(f"ip netns exec {ns_name} ip link del {dev_name}")
